        ...
        <AnimatePresence>
          {selected && (
            <motion.div initial={{x:300,opacity:0}} animate={{x:0,opacity:1}} exit={{x:300,opacity:0}}
              style={{width:selected?'45%':'0',display:'flex',flexDirection:'column',overflow:'hidden',background:'#111',borderLeft:'1px solid #222'}}>
              {/* Detail Header */}
              <div style={{padding:'12px 16px',borderBottom:'1px solid #333',flexShrink:0}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:8}}>
                  <div>
                    <div style={{color:'#e8c030',fontWeight:800}}>{t('Table','टेबल')} {selected.tableNumber}</div>
                    <div style={{color:'#aaa',fontSize:13}}>👤 {selected.customerName}</div>
                    <div style={{color:'#888',fontSize:11,marginTop:2}}>📅 {new Date(selected.startTime||selected.createdAt).toLocaleString()}</div>
                  </div>
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    {selected.status==='active'&&[2,3,5].map(min=>(
                      <button key={min} style={{padding:'5px 10px',background:'#d97706',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',fontWeight:700,fontSize:12}}
                        onClick={()=>sendWaitVoice(selected.tableNumber,selected._id,min)}>
                        🔊 {min}m
                      </button>
                    ))}
                    {selected.status==='active'&&(
                      <button style={{padding:'5px 10px',background:'#dc2626',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',fontWeight:700,fontSize:12}}
                        onClick={()=>endSession(selected._id)}>
                        🚪 {t('End','संपवा')}
                      </button>
                    )}
                    <button style={{background:'#333',border:'none',color:'#fff',borderRadius:6,padding:'4px 10px',cursor:'pointer'}} onClick={()=>setSelected(null)}>✕</button>
                  </div>
                </div>
              </div>
              ...
              <div style={{flex:1,overflowY:'auto',padding:14}}>
                {/* Orders summary */}
                <div style={{color:'#e8c030',fontWeight:700,marginBottom:10}}>{t('Orders','ऑर्डर')}</div>
                {orders.filter(o=>o.session===selected._id||o.session?.toString()===selected._id?.toString()).map(order=>(
                  <div key={order._id} style={{background:'#1c1c1c',borderRadius:10,padding:10,marginBottom:8,border:'1px solid #333'}}>
                    <div style={{color:'#e8c030',fontWeight:700,marginBottom:6}}>#{order._id?.slice(-4)} — ₹{order.totalAmount}</div>
                    {order.items.map((item,idx)=>(
                      <div key={idx} style={{display:'flex',alignItems:'center',padding:'4px 0',gap:8,fontSize:13}}>
                        <span style={{flex:1}}>{item.nameMarathi||item.name} x{item.quantity}</span>
                        <select style={{padding:'3px 6px',background:'#252525',border:'1px solid #555',color:'#fff',borderRadius:6,fontSize:11}}
                          value={item.status} onChange={e=>updateItemStatus(order._id,idx,e.target.value,false)}>
                ...
                {/* Conversation */}
                <div style={{color:'#4ade80',fontWeight:700,margin:'14px 0 8px'}}>{t('💬 Conversation','💬 संभाषण')}</div>
                <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:10}}>
                  {(selected.messages||[]).map((msg,i)=>(
                    <div key={i} style={{...S.chatBubble,...(msg.sender==='admin'?S.bubbleAdmin:S.bubbleCustomer)}}>
                      <div style={S.chatSender}>{msg.sender==='admin'?'👨‍💼 Admin':`🙋 ${selected.customerName}`}</div>
                      <div style={{fontSize:13}}>{msg.text}</div>
                      {msg.waitTime&&<div style={{color:'#e8c030',fontSize:11}}>⏱ {msg.waitTime} min</div>}
                      <div style={S.chatTime}>{new Date(msg.timestamp).toLocaleTimeString()}</div>
                    </div>
                  ))}
                  <div ref={msgEndRef}/>
                ...
                {selected.status==='active' && (
                  <div style={{display:'flex',gap:8}}>
                    <input style={S.chatInp} placeholder={t('Message to customer...','ग्राहकाला संदेश...')}
                      value={chatMsg} onChange={e=>setChatMsg(e.target.value)}
                      onKeyPress={e=>e.key==='Enter'&&sendAdminMsg(selected._id,selected.tableNumber)}/>
                    <button style={S.sendBtn} onClick={()=>sendAdminMsg(selected._id,selected.tableNumber)}>{t('Send','पाठवा')}</button>
                  </div>
                )}
              </div>
            </motion.div>
