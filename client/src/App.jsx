...
function speak(text, lang = 'mr') {
  if (!('speechSynthesis' in window)) return
  window.speechSynthesis.cancel()
  if (_voices.length === 0) _loadVoices()
  const u = new SpeechSynthesisUtterance(text)
  const preferred =
    _voices.find(v => v.name.includes('Kalpana')) ||
    _voices.find(v => v.name.includes('Hemant')) ||
    _voices.find(v => v.name.toLowerCase().includes('microsoft') && v.lang.includes('hi')) ||
    _voices.find(v => v.name.includes('Google') && v.lang.includes('hi')) ||
    _voices.find(v => v.lang === 'hi-IN') ||
    _voices.find(v => v.lang.includes('hi')) ||
    _voices.find(v => v.lang === 'en-IN') ||
    null
  if (preferred) u.voice = preferred
  u.lang  = lang === 'mr' ? 'hi-IN' : 'en-IN'
  u.rate  = 0.92
  u.pitch = 1.05
  u.volume = 1.0
...
function useFlash() {
  ...
  const add = useCallback((text, type = 'info', waitTime = null, duration = 6000) => {
    const id = Date.now() + Math.random()
    setFlashes(p => [...p, { id, text, type, waitTime }])
    setTimeout(() => setFlashes(p => p.filter(f => f.id !== id)), duration)
  }, [])
  const remove = useCallback((id) => setFlashes(p => p.filter(f => f.id !== id)), [])
  return { flashes, add, remove }
}

function FlashStack({ flashes, onRemove }) {
  const colors  = { success:'#16a34a', error:'#dc2626', info:'#2563eb', wait:'#d97706', remind:'#7c3aed', notreceived:'#dc2626', warning:'#d97706' }
  const icons   = { success:'✅', error:'❌', info:'ℹ️', wait:'⏱', remind:'🔔', notreceived:'🚨', warning:'⚠️' }
  const labels  = { success:'सूचना', error:'त्रुटी', info:'माहिती', wait:'व्यवस्थापकाचा संदेश', remind:'आठवण', notreceived:'मिळाले नाही', warning:'सावधान' }
  return (
    <div style={{ position:'fixed', top:12, right:12, zIndex:9999, display:'flex', flexDirection:'column', gap:8, maxWidth:340, pointerEvents:'none' }}>
      <AnimatePresence>
        {flashes.map(f => (
          <motion.div key={f.id}
            ...
            style={{ background:'#1c1c1c', border:`2px solid ${colors[f.type]||'#444'}`, borderRadius:14,
              padding:'12px 38px 12px 14px', boxShadow:`0 4px 24px rgba(0,0,0,.5)`, position:'relative', pointerEvents:'all' }}>
            <div style={{ display:'flex', alignItems:'flex-start', gap:8 }}>
              <span style={{ fontSize:18, flexShrink:0 }}>{icons[f.type]||'ℹ️'}</span>
              <div>
                <div style={{ color:colors[f.type], fontWeight:700, fontSize:12, marginBottom:3 }}>{labels[f.type]||'सूचना'}</div>
                <div style={{ color:'#fff', fontSize:14, lineHeight:1.4 }}>{f.text}</div>
                {f.waitTime && <div style={{ color:'#fbbf24', fontSize:13, fontWeight:700, marginTop:4 }}>⏱ {f.waitTime} मिनिटांत येईल</div>}
              </div>
            </div>
            <button onClick={() => onRemove(f.id)}
              style={{ position:'absolute', top:8, right:10, background:'none', border:'none', color:'#666', cursor:'pointer', fontSize:14 }}>✕</button>
          </motion.div>
        ))}
      </AnimatePresence>
    </div>
...
const SESSION_KEY = (t) => `pahunchar_session_table_${t}`
...
export function CustomerApp({ tableNumber }) {
  ...
  const saved = (() => { try { return JSON.parse(localStorage.getItem(SESSION_KEY(tableNumber)) || 'null') } catch { return null } })()
  ...
  const { flashes, add: addFlash, remove: removeFlash } = useFlash()
  const socketRef = useRef(null)
  const msgEndRef = useRef(null)

  // ── notify: flash + voice together ───────────────
  const notify = useCallback((text, type = 'info', waitTime = null, duration = 7000) => {
    addFlash(text, type, waitTime, duration)
    speak(text, 'mr')
  }, [addFlash])
  ...
  useEffect(() => {
    if (phase === 'active' && session?._id) {
      apiCall(`/orders/session/${session._id}`)
        .then(d => { setOrders(d.orders||[]); setBasicOrders(d.basicOrders||[]) })
        .catch(() => {})
    }
  }, [session?._id])
  ...
  async function sendJoinRequest() {
    ...
    try {
      const data = await apiCall('/sessions/request','POST', { tableNumber, customerName:customer.name, customerPhone:customer.phone, language:lang })
      setSession(data.session); setPhase('waiting')
  ...
  function addToCart(item, qty) {
    setCart(prev => { const ex=prev.find(c=>c.id===item.id); return ex ? prev.map(c=>c.id===item.id?{...c,qty:c.qty+qty}:c) : [...prev,{...item,qty}] })
  }
  ...
  async function placeOrder() {
    ...
    try {
      ...
      notify(t('ऑर्डर दिली! थोड्या वेळात येईल.','Order placed!'), 'success')
    } catch(e) { alert('Error: '+e.message) }
  }

  function addToBasicCart(item) {
    setBasicCart(prev => { const ex=prev.find(c=>c.id===item.id); return ex ? prev.map(c=>c.id===item.id?{...c,qty:c.qty+1}:c) : [...prev,{...item,qty:1}] })
  }
  ...
  async function placeBasicOrder() {
    ...
    try {
      ...
      const order = await apiCall('/orders/basic','POST', { sessionId:session._id, mainOrderId:orders[0]?._id, tableNumber, customerName:customer.name, items })
      ...
      notify(t(`${items.length} वस्तू मागवल्या!`,`${items.length} items ordered!`), 'success')
    } catch(e) { alert('Error: '+e.message) }
  }
  ...
  async function markItem(orderId, itemIndex, status, isBasic) {
    try {
      ...
      const updater = prev => prev.map(o => { if(o._id!==orderId)return o; const items=[...o.items]; items[itemIndex]={...items[itemIndex],status}; return{...o,items} })
      isBasic ? setBasicOrders(updater) : setOrders(updater)

      if (status === 'not-received') {
        const allOrders  = isBasic ? basicOrders : orders
        const order      = allOrders.find(o => o._id === orderId)
        const itemName   = order?.items[itemIndex]?.nameMarathi || order?.items[itemIndex]?.name || 'वस्तू'
        const notifText  = `टेबल ${tableNumber}: "${itemName}" मिळाली नाही — व्यवस्थापकाला कळवले`
        notify(notifText, 'notreceived', null, 10000)
        socketRef.current?.emit('item-not-received', { tableNumber, itemName, sessionId:session._id, isBasic })
        socketRef.current?.emit('send-message-to-admin', { tableNumber, message:`🚨 "${itemName}" मिळाली नाही!`, sessionId:session._id })
        await apiCall(`/sessions/${session._id}/message`,'POST',{ sender:'customer', text:`🚨 "${itemName}" मिळाली नाही!` })
        setMessages(prev => [...prev, { sender:'customer', text:`🚨 "${itemName}" मिळाली नाही!`, timestamp:new Date() }])
      }
      ...
      if (status === 'received') {
        ...
        const order     = allOrders.find(o => o._id === orderId)
        const itemName  = order?.items[itemIndex]?.nameMarathi || order?.items[itemIndex]?.name || 'वस्तू'
        notify(`${itemName} मिळाली! ✓`, 'success')
        socketRef.current?.emit('item-received', { orderId, itemIndex, isBasic, tableNumber, itemName })
      }
  ...
  async function remindAdmin(orderId, itemName) {
    ...
    const msg = t(`टेबल ${tableNumber}: "${itemName}" अजून आली नाही — कृपया पाठवा`, `Table ${tableNumber}: "${itemName}" not received yet`)
    socketRef.current?.emit('send-message-to-admin', { tableNumber, message:msg, sessionId:session._id })
    await apiCall(`/sessions/${session._id}/message`,'POST',{ sender:'customer', text:msg })
    setMessages(prev => [...prev, { sender:'customer', text:msg, timestamp:new Date() }])
    setRemindCooldown(p => ({ ...p, [orderId]:true }))
    notify(t('व्यवस्थापकाला आठवण दिली!','Reminder sent!'), 'remind')
    setTimeout(() => setRemindCooldown(p => ({ ...p, [orderId]:false })), 60000)
  }

  async function sendMessage() {
    if (!msgInput.trim() || !session) return
    const text = msgInput
    setMessages(prev => [...prev, { sender:'customer', text, timestamp:new Date() }])
    socketRef.current?.emit('send-message-to-admin', { tableNumber, message:text, sessionId:session._id })
    await apiCall(`/sessions/${session._id}/message`,'POST',{ sender:'customer', text })
    setMsgInput('')
  }

  async function leaveTable() {
    try { await apiCall(`/sessions/${session._id}/end`,'PUT') } catch {}
  ...
  if (phase === 'join') return (
    <div style={S.screen}>
      <FlashStack flashes={flashes} onRemove={removeFlash}/>
      <button style={{ ...S.langBtn, position:'absolute', top:18, right:18 }} onClick={() => setLang(l => l==='mr'?'en':'mr')}>
      ...
      <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} style={S.joinCard}>
        ...
        <input style={S.inp} placeholder={t('आपले नाव *','Your name *')} value={customer.name}
          onChange={e => setCustomer(p=>({...p,name:e.target.value}))} onKeyPress={e=>e.key==='Enter'&&sendJoinRequest()}/>
        <input style={S.inp} placeholder={t('मोबाईल (ऐच्छिक)','Mobile (optional)')} value={customer.phone}
          onChange={e => setCustomer(p=>({...p,phone:e.target.value}))}/>
        <div style={{ display:'flex', gap:8, marginBottom:12 }}>
          {['mr','en'].map(l => (
            <button key={l} style={{ ...S.ghostBtn, flex:1, borderColor:lang===l?'#e8c030':'#444', color:lang===l?'#e8c030':'#888' }}
              onClick={() => setLang(l)}>{l==='mr'?'मराठी':'English'}</button>
          ))}
        </div>
  ...
  if (phase === 'waiting') return (
    <div style={S.screen}>
      ...
      <motion.div style={S.waitCard} initial={{ scale:0.8 }} animate={{ scale:1 }}>
        <motion.div animate={{ rotate:360 }} transition={{ repeat:Infinity, duration:2, ease:'linear' }} style={{ fontSize:52 }}>⏳</motion.div>
    ...
    <div style={S.app}>
      ...
      <div style={S.header}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button style={S.leaveBtn} onClick={() => setShowLeave(true)}>🚪 {t('सोडा','Leave')}</button>
        </div>
      </div>
          ...
          {activeTab==='chat' && (
            <motion.div key="chat" initial={{ opacity:0 }} animate={{ opacity:1 }} style={S.chatWrap}>
              <div style={S.chatMsgs}>
                ...
                {messages.map((msg,i) => (
                  <div key={i} style={{ ...S.chatBubble, ...(msg.sender==='customer'?S.bubbleCustomer:S.bubbleAdmin) }}>
                    ...
                    <div>{msg.text}</div>
                    {msg.waitTime && <div style={{ color:'#fbbf24', fontSize:12, marginTop:4 }}>⏱ {msg.waitTime} {t('मिनिटांत येईल','min wait')}</div>}
              ...
              <div style={S.chatInputRow}>
                <input style={S.chatInp} placeholder={t('संदेश लिहा...','Type a message...')} value={msgInput}
                  onChange={e => setMsgInput(e.target.value)} onKeyPress={e => e.key==='Enter'&&sendMessage()}/>
  ...
  return (
    <motion.div whileHover={{ scale:1.005 }} style={S.menuCard}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:12 }}>
        {qty === 0 ? (
          <button style={{ ...S.addBtn, width:'100%', padding:'10px 0' }} onClick={() => { onAdd(item,1); setQty(1) }}>+ {t('टाका','Add')}</button>
        ) : (
          <div style={S.qtyCtrl}>
            <button style={S.qtyBtn} onClick={() => { if(qty>1){onAdd(item,-1);setQty(q=>q-1)}else setQty(0) }}>−</button>
            <span style={S.qtyNum}>{qty}</span>
            <button style={S.qtyBtn} onClick={() => { onAdd(item,1); setQty(q=>q+1) }}>+</button>
          </div>
        )}
      </div>
    </motion.div>
...
function OrderCard({ order, lang, t, onGotIt, onNotReceived, onRemind, remindCooldown }) {
  ...
  useEffect(() => {
    const check = () => {
      const now = Date.now(); const t2 = {}
      order.items.forEach((item,idx) => { t2[idx] = (now - new Date(item.orderedAt||order.orderedAt||order.createdAt).getTime()) > 5*60*1000 })
      setTimers(t2)
    }
    check()
        ...
        <div key={idx} style={{ padding:'8px 0', borderBottom:'1px solid #2a2a2a' }}>
          ...
          {(item.status==='pending'||item.status==='preparing') && (
            <motion.button whileTap={{ scale:0.9 }}
              style={{ ...S.remindBtn, opacity:remindCooldown?0.5:1, cursor:remindCooldown?'not-allowed':'pointer' }}
              onClick={() => !remindCooldown && onRemind(idx)}>
              🔔 {t('व्यवस्थापकाला आठवण द्या','Remind Admin')}{remindCooldown?` (${t('1 मिनिट','1 min')})` : ''}
            </motion.button>
          )}
        </div>
      ))}
...
function BasicItemCard({ item, lang, qty, onAdd, onRemove }) {
  return (
    <div style={S.basicCard}>
      ...
      <div style={S.qtyCtrl}>
        {qty > 0
          ? <><button style={S.qtyBtn} onClick={onRemove}>−</button><span style={S.qtyNum}>{qty}</span><button style={S.qtyBtn} onClick={onAdd}>+</button></>
...
function BasicOrderCard({ order, lang, t, onGotIt, onNotReceived, onRemind, remindCooldown }) {
  ...
  useEffect(() => {
    const check = () => {
      const now = Date.now(); const t2 = {}
      order.items.forEach((item,idx) => { t2[idx] = (now - new Date(item.orderedAt||order.orderedAt||order.createdAt).getTime()) > 60*1000 })
      setTimers(t2)
    }
    check()
...
export function AdminApp() {
  ...
  // ── Admin voice+flash notify ──────────────────────
  const voiceNotify = useCallback((text, type = 'info') => {
    speak(text, 'mr')
    const id = Date.now()
    setNotifQueue(prev => [...prev, { id, text, type }])
    setTimeout(() => setNotifQueue(prev => prev.filter(n => n.id !== id)), 8000)
  }, [])
  ...
  async function fetchAll() {
    try {
      const [sAll, oData] = await Promise.all([ apiCall('/sessions'), apiCall('/orders/active') ])
      setSessions(sAll)
  ...
  async function loadQRCodes() {
    try { setQrData(await apiCall('/qr/all/print')) } catch(e) { console.error(e) }
  }
  ...
  useEffect(() => {
    ...
    socket.on('new-session-request', data => {
      ...
      setPendingReqs(prev => [s, ...prev.filter(r => r._id !== s._id)])
      voiceNotify(`टेबल ${s.tableNumber} वरून ${s.customerName} यांची बसण्याची विनंती आली आहे`, 'info')
    })

    socket.on('voice-notify-admin', ({ text, type }) => voiceNotify(text, type || 'info'))

    socket.on('new-order', data => {
      setOrders(prev => [data.order, ...prev.filter(o => o._id !== data.order._id)])
      voiceNotify(`टेबल ${data.order.tableNumber} वरून ${data.order.items?.length||''} वस्तूंची ऑर्डर आली आहे`, 'info')
    })

    socket.on('new-basic-order', data => {
      setBasicOrders(prev => [data.order, ...prev.filter(o => o._id !== data.order._id)])
      voiceNotify(`टेबल ${data.order.tableNumber} वरून ${data.order.items?.length||''} वस्तूंची मागणी आली आहे`, 'info')
    })

    socket.on('message-from-customer', ({ tableNumber, message, sessionId }) => {
      voiceNotify(`टेबल ${tableNumber} वरून संदेश: ${message}`, 'remind')
      const msgObj = { sender:'customer', text:message, timestamp:new Date() }
      setSessions(prev => prev.map(s => s._id===sessionId ? { ...s, messages:[...(s.messages||[]),msgObj] } : s))
      if (selected?._id === sessionId) setSelected(prev => prev ? { ...prev, messages:[...(prev.messages||[]),msgObj] } : prev)
    })

    socket.on('item-not-received', ({ tableNumber, itemName }) => {
      voiceNotify(`तातडी! टेबल ${tableNumber}: "${itemName}" मिळाली नाही. लगेच पाठवा.`, 'notreceived')
    })

    socket.on('item-received', ({ tableNumber, itemName }) => {
      if (itemName) voiceNotify(`✅ टेबल ${tableNumber}: "${itemName}" मिळाली`, 'success')
    })
    ...
    return () => socket.disconnect()
  }, [isLoggedIn, voiceNotify, selected?._id])
  ...
  async function sendWaitVoice(tableNumber, sessionId, waitMin) {
    ...
    setSessions(prev => prev.map(s => s._id===sessionId ? { ...s, messages:[...(s.messages||[]),{sender:'admin',text:msg,waitTime:waitMin,timestamp:new Date()}] } : s))
    if (selected?._id === sessionId) setSelected(prev => prev ? { ...prev, messages:[...(prev.messages||[]),{sender:'admin',text:msg,waitTime:waitMin,timestamp:new Date()}] } : prev)
  ...
  async function sendAdminMsg(sessionId, tableNumber) {
    ...
    const msgObj = { sender:'admin', text, timestamp:new Date() }
    socketRef.current?.emit('send-message-to-table', { tableNumber, message:text, type:'text' })
    await apiCall(`/sessions/${sessionId}/message`,'POST',{ sender:'admin', text })
    setSessions(prev => prev.map(s => s._id===sessionId ? { ...s, messages:[...(s.messages||[]),msgObj] } : s))
    if (selected?._id === sessionId) setSelected(prev => prev ? { ...prev, messages:[...(prev.messages||[]),msgObj] } : prev)
  }
  ...
  if (!isLoggedIn) return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#0a0a0a,#1a0000)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }}
        style={{ background:'rgba(255,255,255,.05)', border:'1px solid rgba(200,165,32,.3)', borderRadius:24, padding:'40px 32px', width:'100%', maxWidth:380 }}>
        ...
        <input style={S.inp} type="password" placeholder="Password" value={loginData.password}
          onChange={e => setLoginData(p=>({...p,password:e.target.value}))} onKeyPress={e=>e.key==='Enter'&&login()}/>
    ...
    <div style={{ minHeight:'100vh', background:'#0a0a0a', display:'flex', flexDirection:'column', color:'#fff', fontFamily:'Mukta,sans-serif' }}>
      ...
      <AnimatePresence>
        {notifQueue[0] && (
          <motion.div initial={{ opacity:0, y:-40 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-40 }}
            style={{ background: notifQueue[0].type==='notreceived'?'#7f1d1d':notifQueue[0].type==='remind'?'#4c1d95':'#7b1111',
              padding:'10px 20px', textAlign:'center', color:'#e8c030', fontWeight:700, fontSize:15, position:'sticky', top:0, zIndex:1000 }}>
            🔊 {notifQueue[0].text}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div style={{ background:'linear-gradient(135deg,#1a0000,#0a0000)', padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid rgba(200,165,32,.3)' }}>
        <div>
          ...
          <div style={{ color:'#888', fontSize:12 }}>{activeSessions.length} active • {pendingReqs.length} pending</div>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          {pendingReqs.length > 0 && (
            <motion.div animate={{ scale:[1,1.1,1] }} transition={{ repeat:Infinity, duration:1 }}
              style={{ background:'#ef4444', color:'#fff', borderRadius:20, padding:'4px 12px', fontWeight:700, fontSize:13 }}>
              🔔 {pendingReqs.length}
            </motion.div>
          )}
          <button style={S.langBtn} onClick={() => setLang(l => l==='en'?'mr':'en')}>{lang==='en'?'मराठी':'English'}</button>
          <button style={{ ...S.leaveBtn, fontSize:12, padding:'6px 12px' }}
            onClick={() => { setIsLoggedIn(false); localStorage.removeItem('admin-token') }}>Logout</button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', background:'#111', borderBottom:'1px solid #222', overflowX:'auto' }}>
        {[
          ...
          <button key={tab.id} style={{ ...S.tab, ...(adminTab===tab.id?S.tabActive:{}), flex:'none', padding:'11px 16px' }}
            onClick={() => { setAdminTab(tab.id); setHistoryDetail(null); if(tab.id==='qr')loadQRCodes() }}>{tab.label}</button>
        ))}
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'0 12px 20px' }}>
        ...
        {adminTab==='pending' && (
          <div>
            <div style={{ color:'#e8c030', fontWeight:800, fontSize:15, marginBottom:12, paddingTop:14 }}>{t('New Seating Requests','नवीन बसण्याच्या विनंत्या')}</div>
            {pendingReqs.length === 0
              ...
              : pendingReqs.map((req,i) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity:0, x:-20 }} animate={{ opacity:1, x:0 }}
                    style={{ background:'rgba(200,165,32,.05)', border:'1px solid rgba(200,165,32,.2)', borderRadius:12, padding:14, marginBottom:10 }}>
                    ...
                    <div style={{ display:'flex', gap:10 }}>
                      <button style={{ flex:1, padding:10, background:'#16a34a', color:'#fff', border:'none', borderRadius:10, cursor:'pointer', fontWeight:700 }}
                        onClick={() => confirmSession(s._id||req._id)}>✓ {t('Confirm','मंजूर')}</button>
                      <button style={{ flex:1, padding:10, background:'#dc2626', color:'#fff', border:'none', borderRadius:10, cursor:'pointer', fontWeight:700 }}
                        onClick={() => rejectSession(s._id||req._id)}>✗ {t('Reject','नाकार')}</button>
                    </div>
                  </motion.div>
                )
              })
        ...
        {adminTab==='live' && (
          <div style={{ paddingBottom:100 }}>
            <div style={{ color:'#e8c030', fontWeight:800, fontSize:17, marginBottom:16, paddingTop:14 }}>{t('Active Live Orders','सक्रिय लाइव ऑर्डर')}</div>
            {activeSessions.length === 0
              ? <div style={S.empty}><div style={{ fontSize:48 }}>🟢</div><p>{t('No active orders','सध्या कोणतीही ऑर्डर नाही')}</p></div>
              : activeSessions.map(sess => {
                const sessOrders      = orders.filter(o => String(o.session)===String(sess._id) && o.status!=='completed')
                const sessBasicOrders = basicOrders.filter(o => String(o.session)===String(sess._id) && o.status!=='served')
                const isChatOpen      = selected?._id === sess._id
                if (sessOrders.length===0 && sessBasicOrders.length===0) return null
                return (
                  <motion.div key={sess._id} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}
                    style={{ background:'#1c1c1c', borderRadius:16, padding:16, marginBottom:24, border:'1px solid rgba(200,165,32,.25)' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16, gap:8 }}>
                      <span style={{ color:'#e8c030', fontWeight:700, fontSize:17 }}>टेबल {sess.tableNumber} — {sess.customerName}</span>
                      <div style={{ display:'flex', gap:8 }}>
                        {[2,3,5].map(m => (
                          <button key={m} style={{ padding:'7px 12px', background:'#d97706', color:'#fff', border:'none', borderRadius:8, cursor:'pointer', fontSize:13, fontWeight:700 }}
                            onClick={() => sendWaitVoice(sess.tableNumber, sess._id, m)}>🔊 {m}m</button>
                        ))}
                        <button style={{ padding:'7px 14px', background:isChatOpen?'#ef4444':'#7b1111', color:'#e8c030', border:'none', borderRadius:8, cursor:'pointer', fontWeight:600 }}
                          onClick={() => setSelected(isChatOpen?null:sess)}>
                          {isChatOpen?'✕ बंद':'💬 चॅट'}
                        </button>
                        <button style={{ padding:'7px 12px', background:'#dc2626', color:'#fff', border:'none', borderRadius:8, cursor:'pointer', fontWeight:600 }}
                          onClick={() => endSession(sess._id)}>🚪 संपवा</button>
                      </div>
                    </div>
                    ...
                    {sessOrders.length > 0 && (
                      <div style={{ marginBottom:16 }}>
                        ...
                        {sessOrders.map(order => (
                          <AdminOrderCard key={order._id} order={order} lang={lang} t={t}
                            onUpdateItem={(idx,status) => updateItemStatus(order._id, idx, status, false)}
                            onWaitVoice={min => sendWaitVoice(order.tableNumber, sess._id, min)}/>
                        ))}
                      </div>
                    )}

                    {sessBasicOrders.length > 0 && (
                      <div>
                        <div style={{ color:'#4ade80', fontWeight:600, marginBottom:8 }}>इतर वस्तू</div>
                        {sessBasicOrders.map(order => (
                          <AdminBasicCard key={order._id} order={order} lang={lang} t={t}
                    ...
                    <AnimatePresence>
                      {isChatOpen && (
                        <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:10 }}
                          style={{ marginTop:20, padding:16, background:'#161616', borderRadius:12, border:'1px solid #555' }}>
                          <div style={{ color:'#4ade80', fontWeight:700, marginBottom:12 }}>💬 संभाषण</div>
                          <div style={{ maxHeight:280, overflowY:'auto', marginBottom:14, display:'flex', flexDirection:'column', gap:10 }}>
                            {(sess.messages||[]).map((msg,i) => (
                              <div key={i} style={{ ...S.chatBubble, alignSelf:msg.sender==='admin'?'flex-end':'flex-start',
                                background:msg.sender==='admin'?'rgba(123,17,17,.8)':'rgba(34,197,94,.2)' }}>
                                <div style={S.chatSender}>{msg.sender==='admin'?'👨‍💼 Admin':`🙋 ${sess.customerName}`}</div>
                                <div style={{ marginTop:4 }}>{msg.text}</div>
                                {msg.waitTime && <div style={{ color:'#fbbf24', fontSize:11 }}>⏱ {msg.waitTime} min</div>}
                                <div style={S.chatTime}>{new Date(msg.timestamp).toLocaleTimeString()}</div>
                              </div>
                            ))}
                          </div>
                          <div style={{ display:'flex', gap:8 }}>
                            <input style={S.chatInp} placeholder="संदेश लिहा..." value={chatMsg}
                              onChange={e => setChatMsg(e.target.value)} onKeyPress={e => e.key==='Enter'&&sendAdminMsg(sess._id, sess.tableNumber)}/>
                            <button style={S.sendBtn} onClick={() => sendAdminMsg(sess._id, sess.tableNumber)}>पाठवा</button>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
        ...
        {adminTab==='tables' && (
          <div style={{ padding:'16px 0' }}>
            <div style={{ color:'#e8c030', fontWeight:800, fontSize:16, marginBottom:16 }}>{t('Table Status','टेबल स्थिती')} ({activeSessions.length} सक्रिय)</div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(90px,1fr))', gap:12 }}>
              {Array.from({length:15},(_,i)=>i+1).map(num => {
                const sess = activeSessions.find(s => s.tableNumber===num)
                return (
                  <motion.div key={num} whileTap={{ scale:0.95 }}
                    onClick={() => sess && (setSelected(sess), setAdminTab('live'))}
                    style={{ background:sess?'#1c1c1c':'#161616', border:`2px solid ${sess?'#22c55e':'#444'}`, borderRadius:14,
                      padding:'16px 10px', textAlign:'center', cursor:sess?'pointer':'default', minHeight:92,
                      display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center' }}>
                    <div style={{ fontSize:22, fontWeight:800, color:sess?'#e8c030':'#666', marginBottom:4 }}>T{num}</div>
                    {sess
                      ? <><div style={{ color:'#22c55e', fontSize:13, fontWeight:600 }}>🟢 {sess.customerName?.split(' ')[0]}</div>
                          <div style={{ fontSize:11, color:'#888', marginTop:4 }}>{orders.filter(o=>String(o.session)===String(sess._id)&&o.status!=='completed').length} orders</div></>
        ...
        {adminTab==='qr' && (
          <div style={{ paddingTop:14 }}>
            <div style={{ color:'#e8c030', fontWeight:800, fontSize:15, marginBottom:6 }}>QR Codes</div>
            <p style={{ color:'#888', fontSize:13, marginBottom:16 }}>Download & print for each table.</p>
            {qrData.length === 0
              ? <button style={{ ...S.primaryBtn, width:'auto', padding:'12px 24px' }} onClick={loadQRCodes}>Load QR Codes</button>
              : (
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
                  {qrData.map(({ tableNumber:tn, qrCode, url }) => (
                    <div key={tn} style={{ background:'#1c1c1c', border:'1px solid #333', borderRadius:12, padding:14, textAlign:'center' }}>
                      <div style={{ fontWeight:800, color:'#e8c030', marginBottom:8 }}>Table {tn}</div>
                      <div style={{ background:'#fff', borderRadius:8, padding:8, display:'inline-block', marginBottom:8 }}>
                        <img src={qrCode} alt={`T${tn}`} style={{ width:100, height:100, display:'block' }}/>
                      </div>
                      <div style={{ color:'#555', fontSize:10, marginBottom:8, wordBreak:'break-all' }}>{url}</div>
                      <a href={qrCode} download={`table-${tn}-qr.png`}
                        style={{ display:'block', padding:7, background:'#7b1111', color:'#e8c030', borderRadius:8, textDecoration:'none', fontSize:12, fontWeight:700 }}>
                        ⬇ Download
                      </a>
                    </div>
                  ))}
        ...
        {adminTab==='history' && !historyDetail && (
          <div style={{ paddingTop:14 }}>
            ...
            <input style={{ ...S.inp, marginBottom:12 }} placeholder={t('Search name, table...','नाव, टेबल...')}
            ...
            {sessions
              ...
              .map(sess => (
                <motion.div key={sess._id} whileHover={{ scale:1.005 }}
                  style={{ background:'#1c1c1c', border:'1px solid #333', borderRadius:12, padding:14, marginBottom:10, cursor:'pointer' }}
                  onClick={() => setHistoryDetail(sess)}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                    <span style={{ color:'#e8c030', fontWeight:700 }}>{t('Table','टेबल')} {sess.tableNumber}</span>
                    <span style={{ padding:'2px 10px', borderRadius:10, fontSize:11, fontWeight:700,
                      background:sess.status==='active'?'rgba(34,197,94,.2)':'rgba(100,100,100,.2)',
                      color:sess.status==='active'?'#22c55e':'#888' }}>{sess.status}</span>
                  </div>
                  <div style={{ color:'#fff', marginBottom:4 }}>👤 {sess.customerName}</div>
                  <div style={{ color:'#888', fontSize:12 }}>📅 {new Date(sess.startTime||sess.createdAt).toLocaleString()}</div>
                  <div style={{ color:'#e8c030', fontSize:13, marginTop:4 }}>₹{sess.totalAmount||0} • {sess.orders?.length||0} orders</div>
                </motion.div>
              ))
            }
          </div>
        ...
        {adminTab==='history' && historyDetail && (
          <div style={{ paddingTop:14 }}>
            <button onClick={() => setHistoryDetail(null)}
              style={{ padding:'10px 16px', background:'#333', color:'#fff', border:'none', borderRadius:8, marginBottom:20, cursor:'pointer' }}>
              ← Back
            </button>
            ...
            <div style={{ color:'#888', fontSize:12, marginBottom:20 }}>
              📅 {new Date(historyDetail.startTime||historyDetail.createdAt).toLocaleString()}
              {historyDetail.endTime && ` → ${new Date(historyDetail.endTime).toLocaleString()}`}
            </div>
            <div style={{ color:'#fbbf24', fontWeight:600, marginBottom:12 }}>ऑर्डर इतिहास</div>
            {orders.filter(o => String(o.session)===String(historyDetail._id)).map((order,idx) => (
              <div key={idx} style={{ background:'#1c1c1c', borderRadius:12, padding:14, marginBottom:12 }}>
                <div style={{ color:'#e8c030', fontWeight:700, marginBottom:8 }}>ऑर्डर #{order._id?.slice(-4)} — ₹{order.totalAmount}</div>
                {order.items?.map((item,i) => (
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'5px 0', borderBottom:i<order.items.length-1?'1px solid #333':'none' }}>
                    <span>{item.nameMarathi||item.name} x{item.quantity}</span>
                    <span style={{ color:'#e8c030' }}>₹{item.price*item.quantity}</span>
                  </div>
                ))}
              </div>
            ))}
            <div style={{ color:'#4ade80', fontWeight:700, marginBottom:12, marginTop:20 }}>💬 संभाषण इतिहास</div>
            <div style={{ maxHeight:400, overflowY:'auto', display:'flex', flexDirection:'column', gap:10 }}>
              {(historyDetail.messages||[]).map((msg,i) => (
                <div key={i} style={{ ...S.chatBubble, alignSelf:msg.sender==='admin'?'flex-end':'flex-start',
                  background:msg.sender==='admin'?'rgba(123,17,17,.8)':'rgba(34,197,94,.2)' }}>
                  <div style={S.chatSender}>{msg.sender==='admin'?'👨‍💼 Admin':`🙋 ${historyDetail.customerName}`}</div>
                  <div style={{ marginTop:4 }}>{msg.text}</div>
                  {msg.waitTime && <div style={{ color:'#fbbf24', fontSize:11 }}>⏱ {msg.waitTime} min</div>}
                  <div style={S.chatTime}>{new Date(msg.timestamp).toLocaleTimeString()}</div>
                </div>
...
// ── AdminOrderCard ────────────────────────────────────────
function AdminOrderCard({ order, lang, t, onUpdateItem, onWaitVoice }) {
  ...
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ ...S.orderCard, marginBottom:12 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer', marginBottom:exp?10:0 }} onClick={() => setExp(!exp)}>
        <span style={{ color:'#e8c030', fontWeight:700 }}>{order.customerName}</span>
        <span style={{ color:'#e8c030' }}>₹{order.totalAmount}</span>
        <span style={{ color:'#888', fontSize:11 }}>{new Date(order.orderedAt||order.createdAt).toLocaleTimeString()}</span>
...
function AdminBasicCard({ order, lang, t, onUpdateItem }) {
  return (
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} style={{ ...S.orderCard, borderColor:'#22c55e44', marginBottom:10 }}>
      <div style={{ color:'#4ade80', fontWeight:700, marginBottom:8, fontSize:13 }}>
        {t('Basic Items','इतर मागणी')}
        <span style={{ color:'#888', fontWeight:400, marginLeft:8, fontSize:11 }}>{new Date(order.orderedAt||order.createdAt).toLocaleTimeString()}</span>
      </div>
      {order.items.map((item,idx) => (
        <div key={idx} style={{ display:'flex', alignItems:'center', padding:'5px 0', borderBottom:'1px solid #2a2a2a', gap:8, fontSize:13 }}>
...
const S = {
  ...
  primaryBtn: { width:'100%', padding:14, borderRadius:13, background:'linear-gradient(135deg,#7b1111,#a01515)', color:'#e8c030', fontWeight:800, fontSize:16, border:'1px solid rgba(200,165,32,.4)', cursor:'pointer' },
  ...
  leaveBtn:   { background:'rgba(239,68,68,.15)', border:'1px solid #ef444466', borderRadius:12, padding:'10px 16px', color:'#ef4444', cursor:'pointer', fontSize:14, fontWeight:700, display:'flex', alignItems:'center', gap:6 },
  app:        { minHeight:'100vh', background:'#0d0d0d', display:'flex', flexDirection:'column', color:'#fff', maxWidth:500, margin:'0 auto', paddingBottom:'env(safe-area-inset-bottom)', fontSize:15 },
  ...
  content:    { flex:1, overflowY:'auto', padding:'14px 14px 220px 14px' },
  ...
  qtyCtrl:    { display:'flex', alignItems:'center', gap:12, background:'rgba(255,255,255,.08)', padding:'6px 10px', borderRadius:10 },
  ...
  cartBar:    { position:'fixed', bottom:0, left:0, right:0, maxWidth:500, margin:'0 auto', background:'linear-gradient(135deg,#7b1111,#a01515)', padding:'18px 20px', paddingBottom:'max(18px,env(safe-area-inset-bottom))', display:'flex', justifyContent:'space-between', alignItems:'center', zIndex:300, boxShadow:'0 -10px 35px rgba(0,0,0,.9)', borderTop:'2px solid #e8c030' },
  orderBtn:   { background:'#e8c030', color:'#1a0000', padding:'14px 28px', borderRadius:12, border:'none', fontWeight:800, fontSize:16, cursor:'pointer', minWidth:160 },
  ...
  notGotBtn:  { padding:'7px 13px', background:'#dc2626', color:'#fff', border:'none', borderRadius:8, cursor:'pointer', fontWeight:700, fontSize:13 },
  ...
  chatBubble: { maxWidth:'82%', padding:'10px 13px', borderRadius:13, wordBreak:'break-word', fontSize:14 },
